...
public abstract class TreeMazeGenerator : BasicMazeGenerator {
	...
	public override void GenerateMaze ()
	{
		...
		{
            int firstInt = Random.Range(0, RowCount);
            int secondInt;
            if (firstInt == 0)
            {
                secondInt = Random.Range(1, ColumnCount);
            }
            else
            {
                secondInt = Random.Range(0, ColumnCount);
            }

            if(GetMazeCell(firstInt, secondInt).IsGoal)
			{
                continue;
            }
			else
			{
				GetMazeCell(firstInt, secondInt).IsGoal = true;
				coinsPlaced++;
            }
        }
    }
